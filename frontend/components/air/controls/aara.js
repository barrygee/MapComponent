// ============================================================
// AARA TOGGLE CONTROL (Air-to-Air Refuelling Areas)
// 14 UK AARA polygon zones rendered as dashed lime outlines
// with centred, rotation-corrected HTML label markers.
//
// Depends on:
//   map (global alias), _overlayStates, _saveOverlayStates,
//   window.MapComponent.{computeCentroid, computeTextRotate, computeLongestEdge}
// ============================================================

// ---- AARA zone dataset ----
// GeoJSON FeatureCollection of 14 Air-to-Air Refuelling Area polygons.
// Coordinates are in [lng, lat] order as required by GeoJSON.
const AARA_ZONES = {
...
class AARToggleControl {
    ...
    onAdd(map) {
        ...
        this.button.style.cssText = 'width:29px;height:29px;border:none;background:#000;cursor:pointer;font-size:16px;display:flex;align-items:center;justify-content:center;transition:opacity 0.2s,color 0.2s';
     ...
     * Label rotations are computed in geographic space (computeTextRotate) then corrected
     * in screen space via _updateLabelRotations() on rotate/pitch events.
    ...
    initLayers() {
        ...
        // Subtle lime fill so zone area is visible without obscuring the map
        this.map.addLayer({
        ...
        // Create HTML label markers once — DOM nodes persist across style reloads
        if (!this._labelMarkers) {
            this._labelEdges   = []; // per-zone longest-edge endpoints (for screen-space rotation)
            this._labelEls     = []; // per-zone <div> elements (for rotation updates)

            this._labelMarkers = AARA_ZONES.features.map(f => {
                // Place label at the polygon centroid
                const coords = window.MapComponent.computeCentroid(f.geometry.coordinates);
                // Initial geographic rotation for the label text
                const rotate  = window.MapComponent.computeTextRotate(f.geometry.coordinates);

                // MapLibre sets a CSS transform on the Marker's outer element for positioning —
                // put the zone name on a nested child so our rotation doesn't conflict with it.
                const wrapper = document.createElement('div');
                wrapper.style.cssText = 'pointer-events:none';

                const label = document.createElement('div');
                label.style.cssText = `color:#fff;font-family:monospace;font-size:10px;white-space:nowrap;text-align:center;transform:rotate(${rotate}deg)`;
                label.textContent   = f.properties.name;

                wrapper.appendChild(label);

                // Store edge endpoints and element reference for screen-space rotation updates
                this._labelEdges.push(window.MapComponent.computeLongestEdge(f.geometry.coordinates));
    ...
    /**
     * Recalculate label rotations in screen space after a map rotate or pitch event.
     * Projects the longest-edge endpoints to pixel coordinates, computes their screen angle,
     * and adjusts the rotation so labels always read left-to-right.
     */
    _updateLabelRotations() {
        if (!this._labelEls) return;
        this._labelEls.forEach((el, i) => {
            const [p0, p1] = this._labelEdges[i];
            const s0 = this.map.project(p0); // geographic → screen pixel
            const s1 = this.map.project(p1);
            let angle = Math.atan2(s1.y - s0.y, s1.x - s0.x) * 180 / Math.PI;
            // Flip 180° if the label would read right-to-left
...
aarControl = new AARToggleControl();
map.addControl(aarControl, 'top-right');

// Update label rotations whenever the map is rotated or pitched so they stay readable
