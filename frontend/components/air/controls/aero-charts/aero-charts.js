...
const ALL_AERO_LAYERS = [].concat(...Object.values(AERO_LAYER_GROUPS));
// ---- Visual styles ----
...
class AeroChartsControl extends SentinelControlBase {
    ...
    get buttonLabel() { return 'IFR'; }
    get buttonTitle() { return 'Toggle aeronautical charts'; }
    onInit() {
    ...
    toggle() {
        ...
        if (this.visible) {
            ['low', 'high'].forEach(g => this._applyGroupVisibility(g));
        }
        this.setButtonActive(this.visible);
        _saveOverlayStates();
    }
