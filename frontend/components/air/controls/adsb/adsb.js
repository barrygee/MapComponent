...
class AdsbLiveControl {
    ...
    initLayers() {
        ...
        if (!this._eventsAdded) {
            ...
            const handleHoverEnter = (e) => {
                ...
                if (hoveredFeature)
                    this._showHoverTag(hoveredFeature);
            };
    ...
    _buildTrackingFields(props) {
        ...
        const fields = [];
        if (props.r)
            fields.push({ label: 'REG', value: props.r });
        if (props.t)
            fields.push({ label: 'TYPE', value: props.t });
        ...
        if (props.squawk)
            fields.push({ label: 'SQUAWK', value: props.squawk });
        if (isEmergency)
            fields.push({ label: 'EMRG', value: props.emergency.toUpperCase(), emrg: true });
        if (props.military)
            fields.push({ label: 'CLASS', value: 'MILITARY' });
        const catLabel = this._categoryLabel(props.category);
        if (catLabel)
            fields.push({ label: 'CATEGORY', value: catLabel });
        return fields;
    }
    // ---- Status bar ----
    ...
    _hideStatusBar() {
        if (typeof window._Tracking !== 'undefined')
            window._Tracking.unregister('air');
        if (typeof window._FilterPanel !== 'undefined')
            window._FilterPanel.reposition();
    }
    _updateStatusBar() {
        ...
        if (aircraftFeature)
            window._Tracking.updateFields('air', this._buildTrackingFields(aircraftFeature.properties));
    }
    _handleUntrack() {
        this._followEnabled = false;
        if (this._tagHex) {
    ...
    _buildCallsignLabelEl(props) {
        ...
        el.addEventListener('mouseenter', () => {
            ...
            if (hoveredFeature)
                this._showHoverTag(hoveredFeature, true);
        });
        el.addEventListener('mouseleave', () => this._hideHoverTag());
        ...
        if (this._selectedHex && this._trails[this._selectedHex]) {
            ...
            for (let i = 0; i < pointCount; i++) {
                ...
                trailFeatures.push({
                    ...
                    properties: { alt: trailPoint.alt, opacity: (i + 1) / pointCount, emerg: isEmerg },
                });
            }
        }
    ...
    _interpolatedCoords(hex) {
        if (this._interpolatedFeatures) {
            ...
            if (interpolatedFeature)
                return interpolatedFeature.geometry.coordinates;
        }
        const aircraftFeature = this._geojson.features.find(f => f.properties.hex === hex);
