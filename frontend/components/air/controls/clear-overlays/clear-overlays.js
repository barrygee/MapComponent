...
class ClearOverlaysControl extends SentinelControlBase {
    ...
    get buttonTitle() { return 'Toggle all overlays'; }
    onInit() {
    ...
    _hideAllOverlays() {
        this.savedStates = {
            ...
            aeroCharts: aeroChartsControl ? aeroChartsControl.visible : false,
        };
        if (roadsControl && roadsControl.roadsVisible)
            roadsControl.toggleRoads();
        if (namesControl && namesControl.namesVisible)
            namesControl.toggleNames();
        if (rangeRingsControl && rangeRingsControl.ringsVisible)
            rangeRingsControl.toggleRings();
        if (aarControl && aarControl.visible)
            aarControl.toggle();
        if (awacsControl && awacsControl.visible)
            awacsControl.toggle();
        if (airspaceControl && airspaceControl.visible)
            airspaceControl.setVisible(false);
        if (aeroChartsControl && aeroChartsControl.visible)
            aeroChartsControl.setVisible(false);
        if (airportsControl && airportsControl.visible)
            airportsControl.toggle();
        if (militaryBasesControl && militaryBasesControl.visible)
        ...
        if (adsbControl && adsbControl.visible) {
            ...
            if (!keepTrails) {
                try {
                    adsbControl.map.setLayoutProperty('adsb-trails', 'visibility', 'none');
                }
                catch (e) { }
            }
        }
    ...
    _restoreAllOverlays() {
        ...
        if (militaryBasesControl && savedStates.militaryBases && !militaryBasesControl.visible)
            militaryBasesControl.toggle();
        if (airspaceControl && savedStates.airspace)
            airspaceControl.setVisible(true);
        if (aeroChartsControl && savedStates.aeroCharts)
            aeroChartsControl.setVisible(true);
        ...
        this.button.style.opacity = '0.3';
        this.button.style.color = '#ffffff';
        if (typeof _syncSideMenuForPlanes === 'function')
