 ...
 */
async function _syncOverlayStatesFromBackend(): Promise<void> {
    ...
    if (!ns || !ns.overlayStates) return;

    const backend: Partial<OverlayStates> = ns.overlayStates;
    ...
    if (roadsControl      && roadsControl.roadsVisible          !== _overlayStates.roads)      roadsControl.toggleRoads();
    if (namesControl      && namesControl.namesVisible           !== _overlayStates.names)      namesControl.toggleNames();
    if (rangeRingsControl && rangeRingsControl.ringsVisible      !== _overlayStates.rings)      rangeRingsControl.toggleRings();
    if (aarControl        && aarControl.visible                  !== _overlayStates.aar)        aarControl.toggle();
    if (awacsControl      && awacsControl.visible                !== _overlayStates.awacs)      awacsControl.toggle();
