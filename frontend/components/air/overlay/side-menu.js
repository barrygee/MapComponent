...
(function buildSideMenu() {
    ...
    map.on('zoom', () => {
        if (locActiveZoom === null || locFlying)
            return;
        if (map.getZoom() <= locActiveZoom - 2)
            deactivateLocationBtn();
    });
    map.on('moveend', () => {
        if (locFlying) {
            locFlying = false;
    ...
    // ---- Group 3: airspace toggle + IFR charts flyout ----
    const chartsGroup = makeGroup('sm-group-charts');
    // AS — plain toggle, no flyout
    const airspaceBtn = makeOverlayBtn('AS', '8px', 'AIRSPACE', () => airspaceControl ? airspaceControl.visible : false, () => { if (airspaceControl)
        airspaceControl.toggle(); });
    chartsGroup.appendChild(airspaceBtn);
    // IFR — opens flyout with LOW / HIGH / NAV / APT sub-toggles
    const ifrBtn = makeOverlayBtn('IFR', '8px', 'IFR CHARTS', () => aeroChartsControl ? aeroChartsControl.visible : false, () => { });
    chartsGroup.appendChild(ifrBtn);
    panel.appendChild(chartsGroup);
    // ---- IFR flyout sub-panel ----
    const ifrFlyout = document.createElement('div');
    ifrFlyout.id = 'as-flyout'; // reuse existing CSS
    function makeAsBtn(label, tooltip, onClick) {
        const btn = document.createElement('button');
    ...
    const IFR_GROUPS = [
        { key: 'low', label: 'LOW', tooltip: 'LOW AIRWAYS  (BELOW FL245)' },
        { key: 'high', label: 'HIGH', tooltip: 'HIGH AIRWAYS (FL245+)' },
    ];
    const ifrGroupBtns = new Map();
    IFR_GROUPS.forEach(({ key, label, tooltip }) => {
        const btn = makeAsBtn(label, tooltip, () => {
            if (!aeroChartsControl)
                return;
            aeroChartsControl.toggleGroup(key);
            syncIfrFlyout();
            ifrBtn.classList.toggle('active', aeroChartsControl.visible);
        });
        ifrGroupBtns.set(key, btn);
        ifrFlyout.appendChild(btn);
    });
    document.body.appendChild(ifrFlyout);
    let ifrFlyoutOpen = false;
    function syncIfrFlyout() {
        if (!aeroChartsControl)
            return;
        IFR_GROUPS.forEach(({ key }) => {
            const btn = ifrGroupBtns.get(key);
            if (btn)
                btn.classList.toggle('active', aeroChartsControl.isGroupActive(key));
        });
    }
    function positionIfrFlyout() {
        const rect = ifrBtn.getBoundingClientRect();
        ifrFlyout.style.top = rect.top + 'px';
        ifrFlyout.style.right = (window.innerWidth - rect.left + 4) + 'px';
    }
    function openIfrFlyout() {
        positionIfrFlyout();
        ifrFlyout.classList.add('open');
        ifrFlyoutOpen = true;
        ifrBtn.classList.add('flyout-open');
        syncIfrFlyout();
    }
    function closeIfrFlyout() {
        ifrFlyout.classList.remove('open');
        ifrFlyoutOpen = false;
        ifrBtn.classList.remove('flyout-open');
    }
    ifrBtn.addEventListener('click', () => {
        if (ifrFlyoutOpen) {
            closeIfrFlyout();
        }
        else {
    ...
    document.addEventListener('click', (e) => {
        if (!ifrFlyoutOpen)
            return;
        if (!ifrFlyout.contains(e.target) && e.target !== ifrBtn && !ifrBtn.contains(e.target)) {
            closeIfrFlyout();
        }
    });
    window.addEventListener('resize', () => { if (ifrFlyoutOpen)
        positionIfrFlyout(); });
    // ---- Group 4: overlay toggles ----
    function toggleAdsb() {
        adsbControl.toggle();
        if (adsbLabelsControl)
            adsbLabelsControl.syncToAdsb(adsbControl.visible);
    }
    const overlayGroup = makeGroup();
    const planesBtn = makeOverlayBtn(PLANE_SVG, '8px', 'PLANES', () => adsbControl ? adsbControl.visible : false, () => { toggleAdsb(); syncLabelsBtn(); syncFilterBtn(); }, true);
    planesBtn.classList.add('sm-expanded-only');
    overlayGroup.appendChild(planesBtn);
    const groundBtn = makeOverlayBtn('GND', '8px', 'GROUND VEHICLES', () => adsbControl ? !adsbControl._hideGroundVehicles : true, () => { if (adsbControl)
        adsbControl.setHideGroundVehicles(!adsbControl._hideGroundVehicles); });
    groundBtn.classList.add('sm-expanded-only');
    overlayGroup.appendChild(groundBtn);
    const towerBtn = makeOverlayBtn('TWR', '8px', 'TOWERS', () => adsbControl ? !adsbControl._hideTowers : true, () => { if (adsbControl)
        adsbControl.setHideTowers(!adsbControl._hideTowers); });
    towerBtn.classList.add('sm-expanded-only');
    overlayGroup.appendChild(towerBtn);
    const labelsBtn = makeOverlayBtn('CALL', '8px', 'CALLSIGNS', () => adsbLabelsControl ? adsbLabelsControl.labelsVisible : false, () => { if (adsbLabelsControl)
    ...
    syncLabelsBtn();
    const ringsBtn = makeOverlayBtn('◎', '16px', 'RANGE RING', () => rangeRingsControl ? rangeRingsControl.ringsVisible : false, () => { if (rangeRingsControl)
        rangeRingsControl.toggleRings(); });
    overlayGroup.appendChild(ringsBtn);
    const aarBtn = makeOverlayBtn('=', '16px', 'A2A REFUELING', () => aarControl ? aarControl.visible : false, () => { if (aarControl)
        aarControl.toggle(); });
    overlayGroup.appendChild(aarBtn);
    const awacsBtn = makeOverlayBtn('○', '16px', 'AWACS', () => awacsControl ? awacsControl.visible : false, () => { if (awacsControl)
        awacsControl.toggle(); });
    ...
    const tiltBtn = makeOverlayBtn(CUBE_SVG, '14px', '3D VIEW', () => _tiltActive, () => {
        ...
        if (panel3d)
            panel3d.style.display = _tiltActive ? 'grid' : 'none';
    ...
    function _getTrackedFeature() {
        ...
        if (!hex || !adsbControl || !adsbControl._geojson)
            return null;
    ...
    window._set3DActive = function (active, applyPitch) {
        if (_tiltActive === active && !applyPitch)
            return;
        _tiltActive = active;
        localStorage.setItem('sentinel_3d', _tiltActive ? '1' : '0');
        const panel3d = document.getElementById('map-3d-controls');
    ...
    const cvlBtn = makeOverlayBtn('CVL', '8px', 'AIRPORTS', () => airportsControl ? airportsControl.visible : false, () => { if (airportsControl)
    ...
    const roadsBtn = makeOverlayBtn('R', '14px', 'ROADS', () => roadsControl ? roadsControl.roadsVisible : false, () => { if (roadsControl)
    ...
    ctrl3d.appendChild(make3dBtn('↑', 'TILT UP', () => {
        ...
        if (window._setTargetPitch)
            window._setTargetPitch(p);
