...
(function buildSdrControls() {
    ...
    const modeSelect   = document.getElementById('sdr-mode-select')    as HTMLSelectElement;
    ...
    function tune() {
        ...
        if (window._SdrDisplay) window._SdrDisplay.setFreqMarker(hz);
        displayFreq(hz);
    }
    ...
    modeSelect.addEventListener('change', () => {
        ...
        if (window._SdrAudio) window._SdrAudio.setMode(modeSelect.value);
    });
    ...
    const audioBtn = document.getElementById('sdr-audio-btn') as HTMLButtonElement;
    let _audioRunning = false;

    function _setAudioRunning(running: boolean) {
    ...
    audioBtn.addEventListener('click', async () => {
        if (!_audioRunning) {
            ...
            if (id) document.dispatchEvent(new CustomEvent('sdr-radio-selected', { bubbles: true, detail: { radioId: id } }));
            if (window._SdrAudio) await window._SdrAudio.initAudio();
    ...
    (window as any)._sdrSetAudioBtn = _setAudioRunning;
    ...
    function applyStatus(msg: SdrStatusMsg) {
        ...
        if (msg.connected) {
            ...
            if (window._SdrDisplay) window._SdrDisplay.setFreqMarker(msg.center_hz);
        }
    }
    ...
    (window as any)._sdrPopulateRadios = function(radios: SdrRadio[]) {
        ...
        while (radioSelect.options.length > 1) radioSelect.remove(1);
        ...
        if (current) radioSelect.value = current;
    };
