...
(function buildSdrDisplay() {
    ...
    let _specLayer: string | null = null;
    let _wfLayer:   string | null = null;
    let _nBins      = 0;
    let _centerHz   = 0;
    let _sampleRate = 0;
    let _freqMinMhz = 0;
    ...
    function _ensureLayers(frame: SdrSpectrumFrame) {
        const n = frame.bins.length;
        if (_specLayer && n === _nBins && frame.center_hz === _centerHz && frame.sample_rate === _sampleRate) return;

        if (_specLayer) { specPlot.deoverlay(_specLayer); _specLayer = null; }
        if (_wfLayer)   { wfPlot.deoverlay(_wfLayer);     _wfLayer   = null; }
    ...
    function renderFrame(frame: SdrSpectrumFrame) {
        const n = frame.bins.length;
        if (!n) return;
        _ensureLayers(frame);
    ...
    function _mhzToX(mhz: number): number {
        if (_freqMaxMhz === _freqMinMhz) return 0;
        return ((mhz - _freqMinMhz) / (_freqMaxMhz - _freqMinMhz)) * overlay.width;
    }

    function _xToMhz(x: number): number {
    ...
    function _drawOverlay() {
        ...
        if (!_tunerMhz || _freqMinMhz === _freqMaxMhz) return;
    ...
    type DragMode = 'tune' | 'bw-left' | 'bw-right' | null;
    let _drag: DragMode = null;

    function _hitTest(x: number): DragMode {
        if (!_tunerMhz) return 'tune';
        const cx = _mhzToX(_tunerMhz);
        const lx = _mhzToX(_tunerMhz - _bwMhz / 2);
        const rx = _mhzToX(_tunerMhz + _bwMhz / 2);
        if (Math.abs(x - lx) <= HANDLE_PX) return 'bw-left';
        if (Math.abs(x - rx) <= HANDLE_PX) return 'bw-right';
        return 'tune';
    }

    function _sendTune(mhz: number) {
        if (!_sdrSocket || !_sdrConnected) return;
        const hz = Math.round(mhz * 1e6);
        _sdrSocket.send(JSON.stringify({ cmd: 'tune', frequency_hz: hz }));
        const inp = document.getElementById('sdr-freq-input') as HTMLInputElement | null;
        if (inp) inp.value = mhz.toFixed(4);
    }

    function _applyBandwidth() {
        if (window._SdrAudio) window._SdrAudio.setBandwidthHz(Math.round(_bwMhz * 1e6));
    }
    ...
    overlay.addEventListener('mouseup', () => {
        if (_drag === 'tune') _sendTune(_tunerMhz);
        if (_drag === 'bw-left' || _drag === 'bw-right') _applyBandwidth();
        _drag = null;
    });
    ...
    function setFreqMarker(hz: number) {
        _tunerMhz = hz / 1e6;
        _drawOverlay();
    }
