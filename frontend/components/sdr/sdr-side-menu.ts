...
(function buildSdrSideMenu() {
    ...
    function makeBtn(id: string, icon: string, label: string, tooltip: string): HTMLButtonElement {
    ...
    btnScan.addEventListener('click', () => {
        if (_sdrScanActive) {
            ...
            if (window._SdrPanel) window._SdrPanel.setScanStatus(false, null);
            // Trigger stop in panel
            const stopBtn = document.getElementById('sdr-scan-stop') as HTMLButtonElement | null;
            if (stopBtn) stopBtn.click();
        } else {
            // Start scan
            btnScan.classList.add('active');
            const startBtn = document.getElementById('sdr-scan-start') as HTMLButtonElement | null;
    ...
    btnAddFreq.addEventListener('click', async () => {
        ...
        try {
            ...
            // Refresh panel
            if ((window as any)._sdrPanelReload) (window as any)._sdrPanelReload();
        } catch (_) {}
    });
    ...
    btnSettings.addEventListener('click', () => {
        ...
        const settingsBtn = document.querySelector<HTMLButtonElement>('#settings-btn, [data-action="settings"]');
