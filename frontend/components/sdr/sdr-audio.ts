...
(function buildSdrAudio() {
    ...
    let _collectingChunks  = false; // stays true through flush yield after rec_stop
    ...
    constructor() {
        ...
        // Circular PCM buffer — 8s capacity; pre-roll 0.4s before playing
        ...
        this.port.onmessage=(ev)=>{
            ...
            // Recording: capture only when squelch is open
    ...
    // ── Recording API ─────────────────────────────────────────────────────────
    async function startRecording(metadata: SdrRecordingMetadata): Promise<number | null> {
    ...
    async function stopRecording(metadata: SdrRecordingMetadata): Promise<any> {
