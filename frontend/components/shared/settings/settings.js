...
window._SettingsPanel = (function () {
    ...
    function _renderSpaceManualTleControl() {
        ...
        fetchBtn.addEventListener('click', async function () {
            ...
            if (!url) {
                urlStatus.innerHTML = '';
                urlStatus.appendChild(_makeTleStatusBadge('Enter a URL first', 'error'));
                return;
            }
            ...
            try {
                ...
                if (!resp.ok)
                    throw new Error(data.error || resp.statusText);
                urlStatus.appendChild(_makeTleStatusBadge(`${(data.inserted ?? 0) + (data.updated ?? 0)} satellites loaded · ${data.inserted ?? 0} new · ${data.updated ?? 0} updated`, 'ok'));
                document.dispatchEvent(new CustomEvent('tle:refreshStatus'));
            }
            catch (err) {
                urlStatus.appendChild(_makeTleStatusBadge('Error: ' + err.message, 'error'));
            }
            finally {
                fetchBtn.textContent = 'UPDATE TLE';
    ...
    function _renderSpaceTleUncategorisedControl() {
        ...
        async function _saveAll() {
            ...
            rows.forEach(function (row) {
                const sel = row.querySelector('.tle-uncat-select');
                if (sel && sel.value) {
