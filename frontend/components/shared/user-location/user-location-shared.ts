...
interface UserLocationConfig {
    /** CSS class applied to the marker div (and matched in CSS). */
    markerClass: string;
    /** Default map zoom level when flying to user location. */
    flyToZoom: number;
    /** Called after a goTo navigation (optional callback). */
    onGoTo?: () => void;
    /** Called when the marker is placed or moved. Receives [lng, lat]. */
    onPositionSet?: (lng: number, lat: number) => void;
}
...
function _createMarkerElement(markerClass: string): HTMLDivElement {
...
function _buildContextMenu(
    ...
    onSetLocation: (lng: number, lat: number) => void,
): HTMLDivElement {
...
function initUserLocation(config: UserLocationConfig): {
    setLocation: (position: UserPosition) => void;
    goToLocation: () => void;
    getCenter: () => maplibregl.LngLatLike | null;
} {
    let _marker: maplibregl.Marker | null = null;
    let _center: [number, number] | null = null;

    function _updateFooter(lat: number, lng: number): void {
        const el = document.getElementById('footer-location');
        if (el) el.textContent = lat.toFixed(4) + ',  ' + lng.toFixed(4);
    }

    function setLocation(position: UserPosition): void {
        ...
        if (config.onPositionSet) config.onPositionSet(longitude, latitude);
    ...
    function goToLocation(): void {
        if (_center) {
            ...
            if (config.onGoTo) config.onGoTo();
    ...
    function getCenter(): maplibregl.LngLatLike | null {
        return _center;
    }
    ...
    if (_cached) {
        try {
            const parsed = JSON.parse(_cached) as { longitude: number; latitude: number; ts?: number; manual?: boolean };
