...
window._SatInfoPanel = (() => {
    ...
    // ---- Fetch ----
    async function _fetchPasses(noradId) {
        if (_fetchAbort)
            _fetchAbort.abort();
        ...
        try {
            ...
            if (abort.signal.aborted)
                return;
            if (!resp.ok) {
                _showMessage('Could not load passes.');
                return;
            }
    ...
    // ---- UI helpers ----
    function _setStatus(text, loading = false) {
        const el = _getStatus();
        if (!el)
            return;
        el.textContent = text;
    ...
    function _showMessage(text) {
        const list = _getList();
        if (list)
            list.innerHTML = `<div class="sip-message">${text}</div>`;
        _setStatus('', false);
    }
    ...
    // ---- Render ----
    function _renderPasses(passes, computedAt) {
        const list = _getList();
        if (!list)
            return;
    ...
    function _startTick() {
        ...
        _tickInterval = setInterval(() => {
            const now = Date.now();
            document.querySelectorAll('.sip-pass-card').forEach(el => {
                const aosMs = parseInt(el.dataset['aosMs'] || '0', 10);
                const losMs = parseInt(el.dataset['losMs'] || '0', 10);
                const cd = el.querySelector('.sip-pass-countdown');
                if (!cd)
                    return;
