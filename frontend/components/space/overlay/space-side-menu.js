...
(function buildSpaceSideMenu() {
    ...
    // Footprint toggle
    const footprintBtn = makeOverlayBtn(FOOTPRINT_SVG, '14px', 'FOOTPRINT', () => issControl ? issControl.footprintVisible : false, () => { if (issControl)
        issControl.toggleFootprint(); _saveSpaceOverlayStates(); }, true);
    issGroup.appendChild(footprintBtn);
    panel.appendChild(issGroup);
    // ---- Group 4: day/night ----
    const dnGroup = makeGroup('ssm-group-daynight');
    const dnBtn = makeOverlayBtn(MOON_SVG, '14px', 'DAY / NIGHT', () => daynightControl ? daynightControl.dnVisible : false, () => { if (daynightControl)
    ...
    filterBtn.addEventListener('click', () => {
        if (window._SpaceFilterPanel)
            window._SpaceFilterPanel.toggle();
    });
