...
export class AdsbLiveControl implements maplibregl.IControl {
    ...
    ;['adsb-trail-line', 'adsb-trail-dots'].forEach((id) => {
      if (this.map.getLayer(id))
        this.map.setLayoutProperty(
          id,
          'visibility',
          (!hidden || isTracking || hasSelection) && !!effectiveTrailHex ? 'visible' : 'none',
        )
    })
  }
  ...
  private _applyTypeFilter(): void {
    ...
    if (this._allHidden && !this._selectedHex) {
      ...
      if (this.map.getLayer('adsb-hit'))
        this.map.setLayoutProperty('adsb-hit', 'visibility', 'none')
      return
    }
    ...
    if (this.map.getLayer('adsb-hit'))
      this.map.setLayoutProperty('adsb-hit', 'visibility', this.visible ? 'visible' : 'none')

    // Isolation mode: show only the selected aircraft
    if (this._isolatedHex) {
      const isolateFilter = ['==', ['get', 'hex'], this._isolatedHex]
      if (this.map.getLayer('adsb-bracket'))
        this.map.setFilter('adsb-bracket', isolateFilter as maplibregl.FilterSpecification)
      if (this.map.getLayer('adsb-hit'))
        this.map.setFilter('adsb-hit', isolateFilter as maplibregl.FilterSpecification)
    ...
    if (this.map.getLayer('adsb-bracket'))
      this.map.setFilter('adsb-bracket', filter as maplibregl.FilterSpecification)
  ...
  _registerIcons(): void {
    ...
    _addOrUpdate('adsb-blip-gnd', createGroundVehicleBlip('#ffffff', 1.1), {
      pixelRatio: 2,
      sdf: false,
    })
    _addOrUpdate('adsb-blip-emerg-gnd', createGroundVehicleBlip('#ff2222', 1.1), {
      pixelRatio: 2,
      sdf: false,
    })
  ...
  initLayers(): void {
    ...
    ;['adsb-icons', 'adsb-hit', 'adsb-bracket', 'adsb-trail-dots', 'adsb-trail-line'].forEach(
      (id) => {
    ...
    this.map.addSource('adsb-trails-source', {
      type: 'geojson',
      data: this._trailsGeojson as GeoJSON.GeoJSON,
    })
    this.map.addSource('adsb-trail-line-source', {
      type: 'geojson',
      data: this._trailLineGeojson as GeoJSON.GeoJSON,
    })
    this.map.addLayer({
      id: 'adsb-trail-line',
      type: 'line',
      source: 'adsb-trail-line-source',
    ...
    this.map.addLayer({
      ...
      layout: {
        ...
        'icon-rotate': [
          'case',
          ['match', ['get', 'category'], ['C1', 'C2', 'C3', 'C4', 'C5'], true, false],
          0,
          ['==', ['get', 't'], 'TWR'],
          0,
          ['get', 'track'],
  ...
  private _buildTagHTML(props: AircraftProperties, forceLeftFacing?: boolean): string {
    ...
    const bellHoverHandlers = notifOn
    ...
    const _buildDataBadges = (isEmerg: boolean, isMil: boolean, leftFacing: boolean): string => {
      ...
      const altStr =
        props.alt_baro && props.alt_baro !== 0 ? this._formatAltBadge(props.alt_baro) : null
      const sqkBadge =
        isEmerg && props.squawk
          ...
          : has('squawk') && props.squawk
            ? _dimBadge('SQK', props.squawk, badgeColor)
            : ''
      const typeBadge =
        has('aircraftType') && props.t
      ...
      const hdgBadge =
        has('heading') && props.track != null
          ? _dimBadge('HDG', Math.round(props.track) + '°', badgeColor)
          : ''
      const spdBadge =
        has('speed') && props.gs != null
          ? _dimBadge('SPD', Math.round(props.gs) + 'kt', badgeColor)
          : ''
  ...
  private _buildTrackingFields(props: AircraftProperties): TrackingField[] {
    ...
    if (isEmergency)
      fields.push({ label: 'EMRG', value: props.emergency.toUpperCase(), emrg: true })
  ...
  private _updateStatusBar(): void {
    ...
    const aircraftFeature = this._geojson.features.find(
      (f) => f.properties.hex === this._selectedHex,
    )
    if (aircraftFeature)
      this._trackingStore.updateFields('air', this._buildTrackingFields(aircraftFeature.properties))
  }
    ...
    if (this._tagHex) {
      if (this._trackingNotifIds && this._trackingNotifIds[this._tagHex]) {
        this._notificationsStore.update({
          id: this._trackingNotifIds[this._tagHex],
          type: 'untrack',
          action: null,
        })
  ...
  private _showHoverTag(
    feature: AircraftGeoFeature,
    fromLabel = false,
    ...
    const hoverLeftFacing =
      fromLabel && labelDir
        ? labelDir === 'left'
        : this._isLeftFacing(feature.properties.track ?? 0)
    ...
    const hoverAnchor = fromLabel
      ...
      : ('top-left' as const)
    const hoverOffset: [number, number] = fromLabel
      ? hoverLeftFacing
        ? [13, 0]
        : [-13, 0]
      : [14, -13]
  ...
  private _buildCallsignLabelEl(props: AircraftProperties): HTMLElement {
    ...
    const makeTrackBtn = () => {
      ...
      btn.textContent = 'TRACKING'
      btn.style.cssText =
        'background:none;border:none;cursor:pointer;padding:0 8px;color:#c8ff00;font-family:inherit;font-size:11px;font-weight:700;letter-spacing:.1em;align-self:stretch;display:flex;align-items:center;white-space:nowrap;'
    ...
    el.addEventListener('mouseenter', () => {
      ...
      if (feature)
        this._showHoverTag(feature, true, el, el.dataset.dir as 'left' | 'right' | undefined)
      if (props.hex) {
  ...
  private _updateCallsignMarkers(): void {
    ...
    for (const f of features) {
      ...
      const isTower =
        ['C3', 'C4', 'C5'].includes(cat) || (f.properties.t || '').toUpperCase() === 'TWR'
      ...
      if (this._callsignMarkers[hex]) {
        ...
        if (box.dataset.dir !== newDir || box.dataset.notif !== newNotif) {
          ...
          this._callsignMarkers[hex] = new maplibregl.Marker({
            element: labelEl2,
            anchor: anchor2,
            offset: offset2,
          })
            ...
            .addTo(this.map)
          if (this._hoverHex === hex && this._hoverMarker) {
            labelEl2.style.visibility = 'hidden'
        ...
        if (isMil) {
          ...
          if (nameSpan)
            nameSpan.style.cssText =
              dimColor + `;padding:${namePad};display:flex;align-items:center;`
          const isTracked = this._followEnabled && hex === this._tagHex
          ...
          if (showAlt && f.properties.alt_baro && f.properties.alt_baro !== 0) {
            if (!altBadge) {
              ...
              altBadge.style.cssText =
                'background:#000000;color:#c8ff00 !important;font-size:12px;font-weight:700;padding:0 7px;letter-spacing:.05em;align-self:stretch;display:flex;align-items:center;gap:4px;'
              altBadge.innerHTML =
                '<span style="opacity:0.45;font-weight:600;font-size:10px;letter-spacing:.12em">ALT</span><span></span>'
              box.insertBefore(
                altBadge,
                box.querySelector('.mil-trk-btn') || box.querySelector('.sqk-badge') || null,
              )
            }
            ;(altBadge.querySelector('span:last-child') as HTMLElement).textContent =
              this._formatAltBadge(f.properties.alt_baro)
          } else if (altBadge) {
            altBadge.remove()
          }
          ...
          if (hasBadge) {
            if (!modelBadge) {
              ...
              box.insertBefore(
                modelBadge,
                box.querySelector('.mil-alt-badge') ||
                  box.querySelector('.mil-trk-btn') ||
                  box.querySelector('.sqk-badge') ||
                  null,
              )
            }
          ...
          if (isTracked && !trkBtn) {
            ...
            trkBtn.style.cssText =
              'background:none;border:none;cursor:pointer;padding:0 8px;color:#c8ff00;font-family:inherit;font-size:11px;font-weight:700;letter-spacing:.1em;align-self:stretch;display:flex;align-items:center;white-space:nowrap;'
            trkBtn.addEventListener('mouseenter', () => {
              ;(trkBtn as HTMLElement).textContent = 'UNTRACK'
            })
            trkBtn.addEventListener('mouseleave', () => {
              ;(trkBtn as HTMLElement).textContent = 'TRACKING'
            })
          ...
          if (showType && f.properties.t) {
            if (!civilBadge) {
              ...
              box.insertBefore(
                civilBadge,
                box.querySelector('.civil-alt-badge') || box.querySelector('.sqk-badge') || null,
              )
            }
          ...
          if (showAlt && f.properties.alt_baro && f.properties.alt_baro !== 0) {
            if (!altBadge) {
              ...
              altBadge.style.cssText =
                'background:rgba(0,0,0,0.5);color:rgba(255,255,255,0.7) !important;font-size:12px;font-weight:700;padding:0 7px;letter-spacing:.05em;align-self:stretch;display:flex;align-items:center;gap:4px;'
              altBadge.innerHTML =
                '<span style="opacity:0.45;font-weight:600;font-size:10px;letter-spacing:.12em">ALT</span><span></span>'
        ...
        if (isEmerg) {
          ...
          if (!badge) {
            ...
            badge.style.cssText =
              'background:#000;color:#ff2222 !important;font-size:12px;font-weight:700;padding:0 7px;letter-spacing:.05em;align-self:stretch;display:flex;align-items:center;'
          ...
          box.style.gap = '5px'
          if (
            !box.querySelector('.mil-model-badge') &&
            !box.querySelector('.mil-trk-btn') &&
            !box.querySelector('.mil-alt-badge') &&
            !box.querySelector('.civil-alt-badge')
          ) {
  ...
  _applySelection(): void {
    ...
    this._updateCallsignMarkers()
    if (this._selectedHex) {
      const selectedFeature = this._geojson.features.find(
        (f) => f.properties.hex === this._selectedHex,
      )
  ...
  private _rebuildTrails(): void {
    ...
    const lineFeatures: GeoJSON.Feature<
      GeoJSON.LineString,
      { emerg: 0 | 1; military: 0 | 1; hex: string }
    >[] = []
    ...
    try {
      if (this.map.getLayer('adsb-trail-line'))
        this.map.setLayoutProperty('adsb-trail-line', 'visibility', vis)
      if (this.map.getLayer('adsb-trail-dots'))
        this.map.setLayoutProperty('adsb-trail-dots', 'visibility', vis)
    } catch (_) {}
    try {
      if (this.map.getSource('adsb-trails-source'))
        (this.map.getSource('adsb-trails-source') as maplibregl.GeoJSONSource).setData(
          this._trailsGeojson as GeoJSON.GeoJSON,
        )
  ...
  private _interpolate(): void {
    ...
    try {
      if (this.map && this.map.getSource('adsb-live')) {
        ;(this.map.getSource('adsb-live') as maplibregl.GeoJSONSource).setData({
          type: 'FeatureCollection',
          features: this._interpolatedFeatures,
        } as GeoJSON.GeoJSON)
      }
    } catch (e) {}
    ...
    if (this._tagMarker && this._tagHex) {
      const interpolatedFeature = this._interpolatedFeatures.find(
        (f) => f.properties.hex === this._tagHex,
      )
      if (interpolatedFeature) {
        ...
        if (this._followEnabled) {
          ...
          this.map.easeTo({
            center: interpolatedFeature.geometry.coordinates,
            pitch: followPitch,
            duration: 150,
            easing: (t) => t,
          })
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      ...
      for (const a of aircraft.filter(
        (a) =>
          a.lat != null &&
          a.lon != null &&
          !['A0', 'B0', 'C0'].includes((a.category || '').toUpperCase()),
      )) {
        ...
        if (hex) {
          ...
          if (justLanded && this._notifEnabled.has(hex)) {
            ...
            this._parkedTimers[hex] = setTimeout(() => {
              ...
              if (this._interpolatedFeatures)
                this._interpolatedFeatures = this._interpolatedFeatures.filter(
                  (f) => f.properties.hex !== hex,
                )
              if (this._callsignMarkers[hex]) {
                this._callsignMarkers[hex].remove()
                delete this._callsignMarkers[hex]
  ...
  private _saveTrackingState(): void {
    try {
      ...
      if (activeHex && this._followEnabled) {
        ...
        const callsign = f
          ? (f.properties.flight || '').trim() || (f.properties.r || '').trim() || ''
          : ''
  ...
  private _doRestoreTracking(): boolean {
    try {
      ...
      this._tagMarker = new maplibregl.Marker({
        element: newEl,
        anchor: trkLeft4 ? 'right' : 'left',
        offset: trkLeft4 ? [13, 0] : [-13, 0],
      })
        .setLngLat(coords)
        .addTo(this.map)
      this._tagHex = hex
      ...
      const is3D = this._is3DActive()
      this.map.easeTo({
        center: aircraftFeature.geometry.coordinates,
        zoom: 16,
        ...(is3D ? { pitch: 45 } : {}),
        duration: 600,
      })
  ...
  toggle(): void {
    ...
    if (this.visible) {
      ...
      for (const [hex, marker] of Object.entries(this._callsignMarkers)) {
        ...
        const isGnd = ['C1', 'C2'].includes(cat)
        const isTower =
          ['C3', 'C4', 'C5'].includes(cat) ||
          (aircraftFeature.properties.t || '').toUpperCase() === 'TWR'
        if (!isGnd && !isTower) {
          marker.remove()
          delete this._callsignMarkers[hex]
        }
